...
class GF_IntersectionOperator : public GF<TFloatType>
{
...
public:
    GF_IntersectionOperator(GF_UPtr<TFloatType> gf0, GF_UPtr<TFloatType> gf1) :
        m_gf0(std::move(gf0)),
        m_gf1(std::move(gf1))
    {
    }
...
protected:
    TFloatType Evaluate_Impl(const sf::Vector2<TFloatType>& position) const override
    {
        ...
        sf::Vector2<TFloatType> gf1Value = m_gf0->Evaluate(position);

        return gf0Value.lengthSquared() > m_gf1Value.lengthSquared() ? gf0Value : gf1Value;
    }
